    ...
    public static String fillZero(String src, int length) {
        ...
        */
        if(src==null || src.length() == length) return src;
            ...
            // 문자열.toCharArray()를 가지고 문자열을 char형의 배열로서 만들 수 있음
            System.arraycopy(src.toCharArray(), 0, c, c.length-src.length(), src.length());

            /*for(int i = 0; i<c.length; i++){
                if(i>=c.length-src.length()){
                    c[i] = src.charAt(i-(c.length-src.length()));
                    str +=c[i];
                }
                else {
                    c[i] = '0';
            ...
            //System.arraycopy(c, c.length-src.length(), src.toCharArray(), 0, src.length());
